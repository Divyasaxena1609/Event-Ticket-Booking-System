...
function ForgotPasswordPage() {
  ...
  const submit = async event => { event.preventDefault(); setSubmitting(true); setError(''); try { const result = await api.forgotPassword(email); setMessage(result?.message || 'If that email has an account, a password reset link has been sent.'); } catch (err) { setError(err.message || 'Unable to send the password reset email.'); } finally { setSubmitting(false); } };
  return <div className="mx-auto flex min-h-screen max-w-md items-center px-6"><form onSubmit={submit} className="panel w-full p-7 sm:p-9"><p className="text-xs font-bold tracking-[.18em] text-brand-600">ACCOUNT RECOVERY</p><h1 className="mt-2 text-3xl font-bold">Reset your password</h1><p className="mt-3 text-sm text-slate-500">Enter your account email and we’ll send you a secure reset link.</p><label className="mt-6 block text-sm font-semibold">Email address<input required type="email" className="field" value={email} onChange={event => setEmail(event.target.value)} /></label>{message && <p className="mt-4 text-sm text-emerald-600">{message}</p>}{error && <p className="mt-4 text-sm text-rose-600">{error}</p>}<button disabled={submitting} className="btn-primary mt-6 w-full disabled:opacity-50">{submitting ? 'Sending…' : 'Email reset link'} <ArrowRight size={17}/></button><p className="mt-6 text-center text-sm text-slate-500"><Link to="/login" className="font-bold text-brand-600">Back to sign in</Link></p></form></div>;
}

function ResetPasswordPage() {
  const navigate = useNavigate(); const token = new URLSearchParams(useLocation().search).get('token') || ''; const [password, setPassword] = useState(''); const [confirmPassword, setConfirmPassword] = useState(''); const [error, setError] = useState(''); const [submitting, setSubmitting] = useState(false);
  const submit = async event => { event.preventDefault(); if (password !== confirmPassword) return setError('Passwords do not match.'); setSubmitting(true); setError(''); try { await api.resetPassword(token, password); navigate('/login', { state: { message: 'Password reset successfully. Please sign in.' } }); } catch (err) { setError(err.message || 'Unable to reset password.'); } finally { setSubmitting(false); } };
  return <div className="mx-auto flex min-h-screen max-w-md items-center px-6"><form onSubmit={submit} className="panel w-full p-7 sm:p-9"><p className="text-xs font-bold tracking-[.18em] text-brand-600">NEW PASSWORD</p><h1 className="mt-2 text-3xl font-bold">Choose a new password</h1><p className="mt-3 text-sm text-slate-500">Your new password must be at least 8 characters.</p><label className="mt-6 block text-sm font-semibold">New password<input required minLength="8" type="password" className="field" value={password} onChange={event => setPassword(event.target.value)} /></label><label className="mt-5 block text-sm font-semibold">Confirm new password<input required minLength="8" type="password" className="field" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} /></label>{!token && <p className="mt-4 text-sm text-rose-600">This password reset link is incomplete.</p>}{error && <p className="mt-4 text-sm text-rose-600">{error}</p>}<button disabled={submitting || !token} className="btn-primary mt-6 w-full disabled:opacity-50">{submitting ? 'Resetting…' : 'Reset password'} <ArrowRight size={17}/></button></form></div>;
}
  ...
  const submit = async e => {
    ...
    try {
      if (register) {
        ...
        const token = await api.register({ email: form.email, password: form.password, userUuid: profile.userUuid });
        localStorage.setItem('accessToken', token.accessToken);
        localStorage.setItem('refreshToken', token.refreshToken);
        const displayUser = toDisplayUser(profile);
