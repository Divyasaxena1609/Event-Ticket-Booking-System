...
function OrganizerConsole({ user }) {
  ...
  const submit = async event => { event.preventDefault(); setSaving(true); setError(''); const payload = { ...form, totalSeats: Number(form.totalSeats), ticketPrice: Number(form.ticketPrice) }; try { if (editing) await api.updateEvent(editing.eventUuid, payload); else await api.createEvent(payload); setOpen(false); setEditing(null); setForm(blank); load(); } catch (err) { setError(err.message || 'Unable to save the event.'); } finally { setSaving(false); } };
